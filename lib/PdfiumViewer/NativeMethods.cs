...
namespace PdfiumViewer
{
    internal static partial class NativeMethods
    {
        ...
        private static IntPtr loadDynamicDll(
            string name_space,
            string platform,
            string pathDllInResourceAssembly,
            string file_name)
        {
            //string file_name = "pdfium.dll";
            ...
            string dirName = Path.Combine(Path.GetTempPath(), name_space +
            ...
            string dllPath = Path.Combine(dirName, file_name);
            if (!File.Exists(dllPath))
            {

                //string resourceName = @"Kit\DLL\" + (IntPtr.Size == 4 ? "x86" : "x64") + @"\" + file_name;
                var assembly = Assembly.GetExecutingAssembly();
                string resourceName = name_space + "." + pathDllInResourceAssembly.Replace(" ", "_").Replace("\\", ".").Replace("/", ".");
        ...
        private static bool TryLoadNativeLibrary(string path)
        {
            ...
            bool ok = File.Exists(path) && LoadLibrary(path) != IntPtr.Zero;
            return ok;
        }

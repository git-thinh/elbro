    ...
    public class IconButton : PictureBox
    {
        ...
        { 
            ...
            using (Stream stream = File.OpenRead("bin/ionicons.ttf"))
            {
                ...
                {
                    bufferFont = new byte[stream.Length];
                    using (MemoryStream ms = new MemoryStream())
                    {
                        int read;
                        while ((read = stream.Read(bufferFont, 0, (int)stream.Length)) > 0)
                            ms.Write(bufferFont, 0, read);
                        bufferFont = ms.ToArray();
                    }
                    //asm = Assembly.Load(bufferFont);
                }
            }
            //return asm;
        }
